...
- **browser-download-warning** - the first Playwright launch is flagged as downloading the browser itself (the setup prompt's smoke test, and `first-run` while that skill exists).
...
**Surfaces resolve at run time.** `first-run` reads `skills/first-run-setup/SKILL.md` **only while that skill exists**. ADR-0001 §7 does not rename it to `orientation` so much as retire it: the mechanical install phase moves into the setup prompt, and what is left is onboarding. Once CORE-114 lands, the surface has no subject and is simply not evaluated - every download it policed is now in the setup document, which `bootstrap-body` checks.
