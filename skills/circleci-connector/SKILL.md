...
Bundled artifacts (read these to verify the SKILL works end-to-end without a live install):

- [`examples/circleci-failed-builds-session.md`](examples/circleci-failed-builds-session.md), full worked transcript: cold start with token paste + chat restart, warm start, multi-project failure summary.
- [`references/circleci-mcp-shape-snapshot.json`](references/circleci-mcp-shape-snapshot.json), captured 12-tool surface and token-capture pattern at the empirical-verification timestamp. Drift-check recipe included.
