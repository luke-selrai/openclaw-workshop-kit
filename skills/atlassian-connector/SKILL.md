...
Atlassian's hosted MCP is a **bridge / proxy OAuth server** (verified live 2026-04-30 against `mcp.atlassian.com/.well-known/oauth-authorization-server`): the discovery document advertises `authorization_endpoint = https://mcp.atlassian.com/v1/authorize`, `token_endpoint = https://cf.mcp.atlassian.com/v1/token`, `registration_endpoint = https://cf.mcp.atlassian.com/v1/register`, S256 PKCE, and public-client `none` auth (issuer `cf.mcp.atlassian.com`). On a hit to `mcp.atlassian.com/v1/authorize`, the server bridges through Atlassian's central OAuth using Atlassian's pre-registered MCP application, then on a successful Allow it issues its own authorization code back to the registered client's redirect URI with the original PKCE/state preserved. From the SKILL's perspective this is a standard OAuth 2.1 + PKCE flow at `mcp.atlassian.com` — Claude Code's MCP runtime drives it natively.

Three practical implications:

- **The consent screen also includes a workspace (site) picker.** Atlassian Cloud accounts can have multiple sites — the user picks which one this connector binds to *on the Allow screen itself*, before clicking Allow. To switch later, re-run Phase 1 Step 3.
- **Organization admin allowlisting is the only hard block.** If the user's Atlassian organization admin (in `admin.atlassian.com`) has restricted third-party app installs or required app review, the consent flow surfaces an "administrator approval required" interstitial instead of completing. There is no API-token fallback for the remote MCP.
...
> "I'll connect your Atlassian now — that covers Jira and Confluence. I'm opening a browser window for you. Please sign in there when it appears, and on the next screen pick which Atlassian workspace you want me to use, then I'll handle the rest. Should take about a minute."
...
- **Logged in** (you see Atlassian's consent UI — a permissions list with a workspace/site picker and Allow / Cancel buttons) → continue to Step 4.
- **Not logged in** (Atlassian sign-in form on `id.atlassian.com` or an SSO redirect) → tell the user, *once*: *"Please sign in to your Atlassian account in the browser window I just opened. On the next screen you'll see an **Allow** button — and above it a picker for which Atlassian workspace to connect. If you have more than one, pick the one with the Jira and Confluence I should work with."* Then `mcp__playwright__browser_wait_for` polling for the consent text (`"would like access"` / `"requesting access"` / `"Accept"` / `"Allow"`) or the admin interstitial text (`"administrator approval"` / `"admin approval required"`). Generous timeout (5 minutes); no nagging. After a long timeout, check in once: *"Still on the sign-in page? Anything I can help with?"*

### Step 4 — Auto-click Allow + auto-detect callback

#### 4a — Confirm workspace selection, narrate, click Allow

Snapshot the consent page. Extract the visible workspace picker selection and the human-readable scope items via `browser_evaluate`:
...
() => {
  ...
  const workspace = sel ? (sel.textContent || sel.value || '').trim().slice(0, 80) : null;
  return { workspace, scopes: items.slice(0, 12) };
}
```

If a workspace value comes back, mention it to the user (one short message, 3-5 representative scopes deduplicated):

> "Atlassian is showing the permissions screen for **\<workspace name\>**. It's asking to: \<scope 1\>, \<scope 2\>, \<scope 3\>. Clicking **Allow** now — if that's not the right workspace, tell me to switch and I'll re-run."

If no workspace value can be extracted (single-site account, picker absent), proceed with a simpler message:
...
Atlassian redirects to Claude Code's localhost callback (Claude Code's MCP runtime owns the listener). Wait for the redirect to complete via `browser_wait_for` on the post-redirect page text. The exact callback page text is not stable across Claude Code 2.x builds — poll for any of several plausible markers, and as a stronger signal also detect a URL change to `localhost`/`127.0.0.1` via a `browser_evaluate`:
...
If either succeeds, treat the callback as complete. If neither succeeds within 5 minutes, check in *once* with the user. Do not nag.

### Step 5 — Detect organization-administrator-approval-required interstitial

After Step 4's Allow click — *or* in the rare case that an Atlassian organization admin has restricted third-party app installs and the consent flow never reached an Allow button — Atlassian may render an interstitial like *"Your administrator must approve this app"*, *"This app requires admin consent"*, or *"Awaiting administrator approval"*. This is a hard block — the user cannot proceed without their Atlassian organization admin allowlisting the Atlassian Remote MCP for the organization.
...
> "Atlassian is telling me your organization administrator needs to allow this connection first. Your Atlassian admin can review and approve the connector for your organization in the connected-apps area of `admin.atlassian.com` (the exact label varies by org tier — usually under Settings → Connected apps or Products → Connected apps; the app appears under the name Atlassian uses for its hosted MCP). Once they've allowed it, come back and say *'connect to my Atlassian'* and I'll finish setting up. There isn't an API-token alternative for this connector, so the admin step is the only way through."

Close the browser, do not retry — the block is org-policy. There is **no API-token fallback** for the remote MCP. **Skip Step 6** in this branch — there is nothing to verify until the admin allows the app.
