...
> **The built-in route is separate.** Phase 1 handles no connection key and starts from the calling Claude account, following its account-matched browser handoff. The golden rule above governs Phase 1-alt only.
...
1. **Built-in connector.** In Desktop, discover Gusto tools by name and description (the prefix can be an opaque `mcp__<id>__`) and inspect its in-app connected state where accessible. Prove it with the read below. In terminal/VS Code only, `claude mcp list` → look for a line starting `claude.ai Gusto`.
   - Connected or tools present → skip to Phase 2. Prove it first with one **read** from the `mcp__claude_ai_Gusto__*` namespace - the company name and how many people are on payroll - before saying so. Never smoke-test with anything that touches a payroll run.
