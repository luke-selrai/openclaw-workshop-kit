...
Identify the calling surface first. Desktop's account and connectors can differ from the standalone CLI, even when `claude auth status` and `claude mcp list` run from Desktop's own Bash. Those commands describe the CLI account; use Desktop's visible account, Connectors view, and actual runtime tools for Desktop evidence. Local Linear credentials are independent of either Claude login.

Run these silently, in order, and act on the first that answers for the user's intended Linear workspace.

1. **Built-in connector.** In Desktop, discover the session's Linear tools by name and description; they can use an opaque `mcp__<id>__` prefix. Check Linear's state in the app's Connectors view where accessible. In terminal/VS Code, run `claude mcp list` and look for `claude.ai Linear` (match the vendor word case-insensitively; there is no `--json` flag).
   - Connected or tools present → prove access with one read through that connector in the calling session: list teams with `limit: 1`. A real answer for the intended workspace is required before saying it is connected and proceeding to **Phase 2**.
   - Reconnect or `! Needs authentication` → use the same account's Connectors view to **Reconnect**, then re-run this check. In Desktop start from the app; in a browser confirm its Claude account matches the caller before opening `https://claude.ai/customize/connectors`.
   - No usable built-in found → continue to step 2. A missing CLI line alone says nothing about Desktop's connector.
2. **The kit's own route.** Discover `mcp__linear__*` tools in this session and run the resume check below if file access is available. If a read works for the intended workspace, keep using it - say *"Linear is already connected"* and skip to **Phase 2**. Do not set the built-in up on top of a working connection. A local read proves this route only, not the Desktop account's built-in connection.
3. **Nothing found** → go to **Phase 1**.

**Precedence note.** In terminal/VS Code, a server registered locally at the same address takes precedence over the built-in and hides it (`/mcp` shows the built-in as hidden). Desktop can expose both the local `mcp__linear__*` tools and its opaque-id built-in tools simultaneously. Discover what is actually callable and retain working connections. Only remove a broken local entry with the user's explicit OK when the built-in is the better route.

**Multiple Linear connections.** Keep each tool's connection attached to its results. A team/issue read proves access, not the signed-in Linear user's identity: an issue's assignee or `gitBranchName` prefix is not an identity check. If the task depends on who is acting, use that connection's authenticated-user read if available, or confirm the intended account with the user. Choose the connection that reaches the requested workspace; do not silently switch connections during a write sequence.

**No shell?** Runtime discovery and reads still apply. Skip unavailable command/file checks; use **Phase 1** only if no working connection is found, then prove the result at Step 5.
...
**Step 1 - Check this session can see built-in connectors.** In Desktop or claude.ai chat, use the calling account's Connectors view and runtime tools; a different terminal login does not gate that account. For terminal/VS Code only, `claude auth status` must show `"authMethod": "claude.ai"`. If it shows anything else, or `~/.claude/settings.json` has `disableClaudeAiConnectors: true`, or `ENABLE_CLAUDEAI_MCP_SERVERS=false` is set in the environment, built-in connectors will not appear in that CLI session. Tell the user in one line that this copy of Claude is signed in a different way, then run **Phase 1-alt** instead.

**Step 2 - Open the connector page for them.** In Desktop, start from **+ → Connectors → Browse connectors → Linear → Connect**, using available computer-use tools or giving that short click sequence if the app is inaccessible. Keep the resulting handoff in a browser signed into the same Claude account as Desktop. For browser or terminal/VS Code setup, use the directory route below in a browser whose Claude account matches the calling session. For that route, say:
...
For the directory route, open `https://claude.ai/directory/linear` (the public mirror of the same page is `https://claude.com/connectors/linear`) in a browser profile signed into the calling Claude account. If the everyday browser matches, open it with `open <url>` on Mac, `xdg-open <url>` on Linux, `start "" <url>` on Windows; otherwise use a matching profile. Confirm the browser's Claude account rather than assuming the everyday profile matches. If the page doesn't load, open `https://claude.ai/customize/connectors` instead and tell them: **Browse**, search "Linear", **Connect**.

> **Why account matching matters here.** Phase 1-alt uses a driven browser to handle its own credential flow. This route handles no connection key, but its browser must match the calling Claude account. A separate signed-in profile is preferable to approving the connection on someone else's everyday account.

**Step 3 - Wait.** Stay hands-off while they sign in. Never ask for a password, a code, or a picture of the sign-in screen.

**Step 4 - Verify.** In Desktop, check Linear's connected state in that app's Connectors view; in browser chat, check that account's view. For terminal/VS Code, run `claude mcp list` again: `claude.ai Linear ... ✔ Connected` passes the account-status check. The CLI branches below apply only to its account; another account's line cannot verify Desktop. Step 5 is required on every surface.
...
> **Which prefix you get.** In terminal/VS Code the built-in tools use `mcp__claude_ai_Linear__*`; Desktop can deliver them as `mcp__<id>__*`. The kit's locally registered tools use `mcp__linear__*`. Both routes reach the same hosted Linear server, so the operation names and tables below apply to both, but each connection has its own authorisation. Discover the actual tools and choose the connection verified in Phase 0; both can be callable in one Desktop session.
