...
The immutable original-context guard applies to provisioning and `finish` recovery. If that context changed before recovery, preserve the changes and review them; never overwrite them or refresh the saved baseline just to pass. Ordinary `check` and `run` instead verify the completed connector's private files, exact identity, ownership, policy and key. Unrelated profile additions, human credential rotation and personal profile-selector changes do not invalidate a completed connection. Each command compares the current personal configuration before and after its entire operation, preserves it, and reports any concurrent change without reverting it. The historical provisioning snapshot stays unchanged.
