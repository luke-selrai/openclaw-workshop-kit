...
description: "Connects Claude Code to a Render.com account by installing the Render CLI, signing the user in, and saving a long-lived API key. Use when the user asks to connect or set up Render, deploy to Render, or wants Render services, logs, Postgres or cron jobs driven from the CLI."
