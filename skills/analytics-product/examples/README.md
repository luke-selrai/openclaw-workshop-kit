...
|------|---------------|-------|
| `posthog-init.py` | A PostHog init plus `track` / `identify` wrapper, env-var loaded, with a clean error if the key is missing | `pip install posthog python-dotenv` and a `.env` with `POSTHOG_API_KEY` |
