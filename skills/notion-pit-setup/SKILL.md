...
description: "Connects a Notion workspace to Claude Code with a Personal Integration Token, so the MCP entry replicates to a headless agent server. Use when the user wants Notion on an agent server or headless install, or asks for the PIT path instead of notion-connector."
