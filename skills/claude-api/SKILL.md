...
## Current Models (cached: 2026-06-16)
...
**ALWAYS use `claude-opus-4-8` unless the user explicitly names a different model.** This is non-negotiable. Do not use `claude-sonnet-4-6`, `claude-sonnet-4-5`, or any other model unless the user literally says "use sonnet" or "use haiku". Never downgrade for cost - that's the user's decision, not yours. Use `claude-fable-5` only when the user explicitly asks for Claude Fable 5, "fable", or Anthropic's most capable model - it has different API behavior than the Opus family (thinking is always on; an explicit `thinking: {type: "disabled"}` returns 400 - omit the parameter instead) and pricing above Opus-tier.
...
**Fable 5 / Opus 4.8 / Opus 4.7 - Adaptive thinking only:** Use `thinking: {type: "adaptive"}`. `thinking: {type: "enabled", budget_tokens: N}` returns a 400 - `budget_tokens` is removed on these models (so are `temperature`, `top_p`, `top_k`). On Opus 4.8 and 4.7, omitting `thinking` or `{type: "disabled"}` both run without thinking; on Fable 5 an explicit `{type: "disabled"}` returns a 400 - omit the parameter entirely instead. **When the user asks for "extended thinking", a "thinking budget", or `budget_tokens`: use `thinking: {type: "adaptive"}` and control depth with `effort` - do NOT use `budget_tokens`, and do NOT switch to an older model.**

**Opus 4.6 / Sonnet 4.6 - Adaptive thinking (recommended):** Use `thinking: {type: "adaptive"}`. Claude dynamically decides when and how much to think. `budget_tokens` is deprecated on Opus 4.6 and Sonnet 4.6 (still functional as a transitional escape hatch, but not for new code) - use adaptive thinking instead. Adaptive thinking also automatically enables interleaved thinking (no beta header needed).

**Effort parameter (GA, no beta header):** Controls thinking depth and overall token spend via `output_config: {effort: "low"|"medium"|"high"|"max"}` (inside `output_config`, not top-level). Default is `high` (equivalent to omitting it). Opus 4.7 added `"xhigh"` (between `high` and `max`) - the best setting for most coding and agentic work on Fable 5 / Opus 4.7 / 4.8, and the default in Claude Code. Supported on Fable 5, Opus 4.5 and later, and Sonnet 4.6; errors on Sonnet 4.5 / Haiku 4.5. Combine with adaptive thinking for the best cost-quality tradeoffs. Use `low` for subagents or simple tasks; `high`/`xhigh` for most intelligence-sensitive work; `max` for the deepest reasoning.
...
**Beta - Fable 5, Opus 4.8, Opus 4.7, Opus 4.6, and Sonnet 4.6.** For long-running conversations that may exceed the context window, enable server-side compaction. The API automatically summarizes earlier context when it approaches the trigger threshold (default: 150K tokens). Requires beta header `compact-2026-01-12`.
...
- **Thinking (Fable 5 / Opus 4.8 / 4.7):** Use `thinking: {type: "adaptive"}` - `budget_tokens` is removed and returns a 400 (so are `temperature`, `top_p`, `top_k`). On Opus 4.6 / Sonnet 4.6 `budget_tokens` is deprecated (transitional only). For older models, `budget_tokens` must be less than `max_tokens` (minimum 1024). This will throw an error if you get it wrong.
...
- **128K output tokens:** Fable 5, Opus 4.6, Opus 4.7, and Opus 4.8 support up to 128K `max_tokens`, but the SDKs require streaming for large `max_tokens` to avoid HTTP timeouts. Use `.stream()` with `.get_final_message()` / `.finalMessage()`.
- **Tool call JSON parsing (4.6 / 4.7 / 4.8 family):** These models may produce different JSON string escaping in tool call `input` fields (e.g., Unicode or forward-slash escaping). Always parse tool inputs with `json.loads()` / `JSON.parse()` - never do raw string matching on the serialized input.
