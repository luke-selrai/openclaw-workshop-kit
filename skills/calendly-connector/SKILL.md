...
**Tool-availability precondition.** On the very first session after `claude mcp add calendly ...`, the deferred-tool reconciliation may not have fired yet, so `mcp__calendly__authenticate` may not be in the tool surface. If that's the case, ask the user *once*: *"I've added Calendly. Please close and reopen the chat once, then say 'connect to my Calendly' and I'll finish."* On resume, Phase 0's resume check sees the `mcpServers.calendly` entry and routes back into Step 3 of this flow.
