...
The user is a non-technical business owner. Claude does the setup using the tools available in the calling session; the user handles sign-in when needed. Every message during either route must follow these rules:

- **You drive, not them.** Use available browser or computer-use tools for navigation and authorised connection approvals. Tool availability comes from this session, not from this skill's allowlist. If a surface is inaccessible, give only the next short click sequence needed there, then resume the work. Ask for sign-in in the opened window when needed; never hand over commands or terminal output.
...
Identify the calling surface first. In Claude Desktop, use the account shown in the app and the tools exposed to this session. A terminal opened from Desktop can be signed into a different Claude account: `claude auth status` and `claude mcp list` describe that terminal account, not Desktop. Keep their evidence separate when the accounts differ or have not been matched.

Run these silently, in order, and act on the first that answers.

1. **Built-in connector.** In Desktop, inspect Shopify in the app's Connectors view and discover Shopify tools in the running session. In terminal or VS Code, use `claude mcp list` and look for `claude.ai Shopify` (match the vendor word case-insensitively).
   - Connected → prove it with one Shopify read in the calling session (read the shop, or list a few products), then skip to PHASE 2. Find Desktop tools by their names and descriptions; they can have an opaque `mcp__<id>__` prefix. Terminal tools use `mcp__claude_ai_Shopify__*`.
   - Reconnect or `! Needs authentication` → follow Phase 1's account-matched route and choose **Reconnect**, then verify again.
   - No connection or tools found → continue. In a chat without connector UI access, try tool discovery first; a missing shell alone does not mean Shopify is disconnected.
2. **The kit's own route.** If this session can run commands, check whether the `shopify` CLI is installed and signed in - Step 1 below does this in full (`shopify version`, then `shopify organization list` returning an org table rather than "you must log in", then a store token for the store the user means). If `shopify store execute --store <intended-store>.myshopify.com --query "{ shop { name } }"` answers for that store, say *"Shopify is already connected"* and skip to PHASE 2. Do not set the built-in up on top of a working connection. This proves the CLI route only; it does not prove a Desktop account's built-in connection.
3. **Nothing found** → Route by need, then Phase 1. Without commands, skip the CLI check and prove the built-in result at Phase 1 Step 5.
...
This is a one-time, once-per-account job. Claude navigates and completes authorised setup wherever its actual tools allow; the user signs in when needed. This skill handles no access token on this route.

**Step 1 - Check the calling session.** In Desktop, use the signed-in account shown in the app and its Connectors view. In claude.ai chat, use that chat's account. Neither is gated by terminal authentication. For terminal or VS Code only, `claude auth status` must show `"authMethod": "claude.ai"`; another auth method, `disableClaudeAiConnectors: true` in `~/.claude/settings.json`, or `ENABLE_CLAUDEAI_MCP_SERVERS=false` means built-in connectors cannot appear in that CLI session. Explain in one plain-English line and use the kit's own route. If the command is unavailable, use the browser and runtime tool checks below.

**Step 2 - Open Shopify from the calling account.** Say: *"I'm connecting Shopify to this Claude account. I'll handle the setup and let you know if you need to sign in."*

- **Desktop first:** composer **+ → Connectors → Browse connectors → Shopify → Connect**. Use computer-use tools if available. If Claude cannot operate the app, say: *"In this Claude window, click **+ → Connectors → Browse connectors**, choose **Shopify**, then **Connect**."* Resume from the resulting browser handoff.
- **Browser or terminal/VS Code:** open `https://claude.ai/directory/shopify` (public listing: `https://claude.com/connectors/shopify`) in a browser signed into the same Claude account as the calling session, then choose **Connect to Claude**. If the page fails to load, use `https://claude.ai/customize/connectors` → **Browse** → search **Shopify** → **Connect**. Use the available browser tools, or `open` on Mac, `xdg-open` on Linux, `start ""` on Windows when only a shell is available.

**Match the browser account before continuing.** The default browser may belong to someone else, even when Desktop is correctly signed in. For a Desktop-created handoff, keep the exact URL opened by **Connect** and use it in a browser profile signed into the same Claude account as Desktop; an isolated browser is suitable when the everyday profile belongs to someone else. Ask the user to sign into that matching account if needed. Preserve the handoff URL and its parameters; do not construct app deep links or substitute a generic directory page. If **Continue connecting** loops, check for this account mismatch before retrying. Once accounts match, retry the handoff; if it has expired, start a fresh **Connect** from Desktop. Do not repeatedly click the looping button or treat it as success.

**Step 3 - Complete Shopify approval.** Use the available browser tools to proceed through setup for the intended store and approve the connection within the user's authorisation. Pause for sign-in if needed; never ask for a password, code, or screenshot of sign-in. If browser control is unavailable, give the next visible action in plain English, such as *"Click **Install** to let Claude connect to your store."* If several stores are available, use the one the user requested or ask which they mean, and confirm the connected store afterwards.

**Step 4 - Verify the intended account.** In Desktop, return to the app's Connectors view and check Shopify's connected state there. In terminal/VS Code, `claude mcp list` must show `claude.ai Shopify … ✔ Connected`; `! Needs authentication` means Reconnect, and no line means the approval has not completed for that account. This command runs fresh each time, so restarting cannot fix a missing line. A successful line from a different terminal account is not Desktop evidence. In a browser-only chat, use its account's Connectors view. If the current tools cannot inspect the relevant view, request only that status check from the user; Step 5 remains required.

**Step 5 - Prove it in the calling session.** Discover and call one actual Shopify read tool (read the shop, or list a few products). Terminal/VS Code tools use `mcp__claude_ai_Shopify__*`; Desktop Code tools can be `mcp__<id>__<tool>` under an opaque id, so find them by names and descriptions and never hard-code the id (it changes on reconnect). Confirm the response belongs to the intended store. Only a real answer counts; a tool error or another account's CLI success is not a completed connection here.

If tools are absent even though Step 4 passed for the same account, recover according to the surface:

- **Terminal or VS Code:** connectors load at session start. Restart the Claude Code session once; in VS Code use **Developer: Reload Window**, then run Phase 0 again.
- **Desktop:** the in-app **+ → Connectors → Browse connectors** route can expose tools in the running session without a restart. Recheck runtime tool discovery after returning to the app. A connection made through a browser directory page can remain invisible even in a new session until Desktop is fully quit and reopened (Mac: **Cmd+Q**; Windows: quit from the tray), then start a new session and run Phase 0. Do not restart to repair an account mismatch.

Perform recovery with available tools, or give the single required action when the calling session cannot do it. If a read still fails, describe the observed limitation and use the kit's route where it covers the user's need; report that route accurately rather than claiming the built-in is connected.
...
- **Auth errors** - on the kit's own route, if you get a 401 or "Unauthorized", re-run the **PHASE 1 - Install & Auth** Step 5 flow. On the built-in Shopify app, follow Phase 1's account-matched route and choose **Reconnect**.
