...
- **The `mcp__docusign__*` tool surface.** That namespace is what the (broken) hosted MCP would have provided. This SKILL doesn't register an MCP server; tool invocations are Bash + curl in Phase 2.
...
### Pre-flight for every Phase 2 call: load env + auto-refresh
...
If the refresh-token rotation 400s (refresh expired after 30 days), surface plainly: *"Your DocuSign sign-in has fully expired. Let me reconnect you."* Walk Phase 1 from Step 5.
