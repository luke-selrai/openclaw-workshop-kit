...
> **Not set up yet?** Walk the user through the four-step install: (1) create a Private Integration Token in GHL, (2) copy the sub-account Location ID, (3) add the `ghl` MCP server entry to `~/.claude.json` (exact shape in the Prerequisites section below), (4) verify with a cheap tool call like listing the first contact.
...
If any of these are missing, walk the user through the four-step install summarised in the Overview callout above, using the MCP server entry shape documented just below. Do not fall back to curl recipes or shell envvars — the MCP config is the only supported install path.
