...
| 7 | `browser_navigate(returned_url)`; user signs in + clicks Allow on DocuSign's consent screen (only manual moment); `browser_wait_for` URL match `localhost:*/callback`; `browser_evaluate(() => window.location.href)` to capture the full callback URL **before** `browser_close` | post-#198 canonical |
