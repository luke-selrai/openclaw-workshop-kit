...
const SCOPES = [
  ...
  'accounting.banktransactions',                  // xero_list_banktx
...
const TOOLS = [
  ...
  {
    name: 'xero_list_banktx',
    description:
      'List bank transactions in Xero. ' +
      'Returns date, type, contact, reference, and amounts.',
    inputSchema: {
      type: 'object',
      properties: {
        page: { type: 'number', description: 'Page number (100 transactions per page)' },
      },
    },
  },
  {
...
async function handleTool(name, args) {
  ...
  switch (name) {
    ...
    case 'xero_list_banktx': {
      ...
      const txs = (res.body.bankTransactions ?? []).map(tx => ({
      ...
      return text({ count: txs.length, bankTransactions: txs });
    }
