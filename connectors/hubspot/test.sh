#!/bin/bash
CONNECTOR="hubspot"; PASS=0; FAIL=0
...
echo "=== Testing HubSpot connector ==="
echo ""
...
if [ -f "$SECRETS_DIR/$CONNECTOR.env" ]; then
  ...
  [ -n "$PRIVATE_APP_ACCESS_TOKEN" ] && [ "$PRIVATE_APP_ACCESS_TOKEN" != "pat-na1-your_token_here" ]; check $? "Access token is set"
else
  check 1 "Access token is set"
fi
