...
CONNECTOR="square"
echo "=== Setting up Square connector ==="
echo ""
...
echo ""
echo "Adding Square MCP server..."
if command -v claude &> /dev/null; then
  if [ -f "$SECRETS_DIR/$CONNECTOR.env" ]; then
    source "$SECRETS_DIR/$CONNECTOR.env" 2>/dev/null || true
  fi
  if [ -n "$SQUARE_ACCESS_TOKEN" ] && [ "$SQUARE_ACCESS_TOKEN" != "your_access_token_here" ]; then
    claude mcp add square -e SQUARE_ACCESS_TOKEN="$SQUARE_ACCESS_TOKEN" -- npx square-mcp-server start 2>/dev/null && echo "[done] MCP server added" || echo "[warn] Could not add MCP server automatically."
  else
