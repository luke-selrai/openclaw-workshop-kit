...
echo "=== Setting up QuickBooks connector ==="
echo ""
...
echo ""
echo "Adding QuickBooks MCP server..."
if command -v claude &> /dev/null; then
  if [ -f "$SECRETS_DIR/$CONNECTOR.env" ]; then
    source "$SECRETS_DIR/$CONNECTOR.env" 2>/dev/null || true
  fi
  if [ -n "$INTUIT_CLIENT_ID" ] && [ "$INTUIT_CLIENT_ID" != "your_client_id_here" ]; then
    claude mcp add quickbooks -e INTUIT_CLIENT_ID="$INTUIT_CLIENT_ID" -e INTUIT_CLIENT_SECRET="$INTUIT_CLIENT_SECRET" -e INTUIT_REDIRECT_URI="${INTUIT_REDIRECT_URI:-http://localhost:3000/callback}" -e INTUIT_ENVIRONMENT="${INTUIT_ENVIRONMENT:-production}" -- npx quickbooks-online-mcp-server 2>/dev/null && echo "[done] MCP server added" || echo "[warn] Could not add MCP server automatically."
  else
